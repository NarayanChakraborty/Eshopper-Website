...
if(isset($_POST['form_add_product'])){
  try {
   ...
   //----------------------------------Image Upload----------------------------
	...
	{
		   $statement3 = $db->prepare("SELECT p_cat_id FROM tbl_products_subcategory where p_subcat_id=?");
           $statement3->execute(array($_POST['product_subcat']));
           $result3 = $statement3->fetchAll(PDO::FETCH_ASSOC);
           foreach ($result3 as $row3) {
			   if($row3['p_cat_id']==$_POST['re'])
			   {
				   	$statement4=$db->prepare("insert into tbl_products(p_name,p_img,p_details,p_price,p_cat_id,p_subcat_id,p_brand_id,p_small,p_medium,p_large,p_arival_date) values(?,?,?,?,?,?,?,?,?,?,?)");
		   $statement4->execute(array($_POST['product_name'],$f1,$_POST['tinyMCE'],$_POST['product_price'],$row3['p_cat_id'],$_POST['product_subcat'],$_POST['brand_name'],$_POST['product_small'],$_POST['product_medium'],$_POST['product_large'],$post_date));
	...
	if((!$_POST['product_subcat'])&& $_POST['re'])
	{
		$statement=$db->prepare("insert into tbl_products(p_name,p_img,p_details,p_price,p_cat_id,p_subcat_id,p_brand_id,p_small,p_medium,p_large,p_arival_date) values(?,?,?,?,?,?,?,?,?,?,?)");
		   $statement->execute(array($_POST['product_name'],$f1,$_POST['tinyMCE'],$_POST['product_price'],$_POST['re'],$_POST['product_subcat'],$_POST['brand_name'],$_POST['product_small'],$_POST['product_medium'],$_POST['product_large'],$post_date));
                                ...
                                <div id="div<?php echo $row1['p_cat_id']; ?>" class="panel-collapse collapse">
                                  <div class="panel-body">
                                    <ul>
                                      ...
                                      <li>
                                        <input type="radio" name="product_subcat" value="<?php echo $row3['p_subcat_id'];?>"required>  <?php echo $row3['p_subcat_name'];?>
                                      </li>
                                     <?php
                                       } 
                                      ?>
                   ...
                   <?php
                      ...
                      if (isset($success_message)) {
                       ...
                       <div class="alert alert-success fade in">
                          <button data-dismiss="alert" class="close close-sm" type="button">X
                            <i class="icon-remove"></i>
                          </button>
